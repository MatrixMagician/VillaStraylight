...
- [x] 20-02-PLAN.md — Runtime zero-outbound RAG smoke harness: pure evalRagSmoke core + liveRagSmoke seam + `villa verify memory` subcommand, negative-control-first (KB-01/02/03, MEM-03, PRIV-05)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 20-03-PLAN.md — On-hardware verification: live OWUI Memory (MEM-01/02/04), auto-extraction default-off (MEM-03), document upload + citations (KB-01/02/03), firewalled zero-outbound proof with negative control (PRIV-05); docs/MEMORY.md
...
- [x] 23-01-PLAN.md — status.Report 2→3: memory rows non-GPU classified (false-green fix), Memory section + skew field, doctor ripple, ONE golden re-freeze (SC#1, CTRL-02)
- [x] 23-02-PLAN.md — backup/restore: Qdrant volume + recall-state.json optional entries, quiesce, manifest model/dim, clean-recreate 2×2 matrix, skew WARN+confirm (SC#2, CTRL-04)
...
| 23. Surfacing, Backup & Memory-Aware Swap | v1.3 | 2/5 | In Progress|  |
