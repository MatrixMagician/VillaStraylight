...
// install_searxng_test.go drives the Phase-29 SearXNG readiness proof: the PURE
// evalSearxngProof verdict core (unit-testable off-hardware via an injected probe).
// Readiness is the REAL format=json query parsing results[] — never a health-200 (the
// project's offload-asserting, never liveness principle). The install-flow wiring
// (Task 2) is TestInstallWebSearchWiring in internal/install/flow_subsystems_test.go.
