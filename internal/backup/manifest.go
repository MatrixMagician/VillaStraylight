...
// manifest.go defines the self-describing backup Manifest: a
// schema-versioned JSON document recording the villa version, host fingerprint,
// both image digests (seam-sourced, never re-typed), the store schema
// versions, per-entry SHA-256 checksums, and the IDENTITIES of the excluded
// model weights (for re-pull; the weights themselves are excluded). The
...
// backupSchemaVersion is the manifest's OWN self-version, INDEPENDENT of
...
//   - v2 (Phase 23, doctrine): adds the OPTIONAL memory entries
...
//   - v3 (Phase 28): adds the OPTIONAL coding-agent coverage
...
//     (bytes never archived, exactly like model weights). Same
...
//   - v4 (Phase 34): adds the OPTIONAL web-search coverage — the
...
//     EXCLUDED by design — no query/URL log entry is ever added. Same
...
const (
	...
	// EntryCrushConfig is the OPTIONAL Phase-28 coding-agent entry:
	...
	// is recorded (ExcludedAgent), mirroring the excluded model weights.
	EntryCrushConfig = "crush.json"
	// EntrySearxngSettings is the OPTIONAL Phase-34 web-search entry: the
	...
	// the mode). Present ONLY in a web-search-on backup; checksummed and
	// SHA-256-verified exactly like every other archive member. Fetched
...
// excludes, recorded so restore can report it for re-pull. It carries
...
// backup deliberately EXCLUDES, recorded so restore can re-stage
// it (re-download the pinned release) and refuse-with-remediation on identity
// drift — exactly the ExcludedModel model-weights pattern. It carries
...
// Plain strings on purpose: the pure core does NOT import internal/detect
...
type Manifest struct {
	...
	// orchestrate.OpenWebUIImage by the caller — NEVER a literal here.
	OpenWebUIImage string `json:"openwebui_image"`
	// ConfigSchemaVersion / UsageSchemaVersion / BenchSchemaVersion are the store
	// schema versions. Plain ints populated from ManifestInput; the REAL
	...
	// LOAD-BEARING vector dimension at backup time (Phase 23), sourced
	...
	// (clone of ExcludedModels weights exclusion). A *ExcludedAgent +
	...
	// LAST field; new fields go ABOVE it.
...
type ManifestInput struct {
	...
	// memory-stack fields: config-sourced model id + dimension and the
...
// output only); the manifest is NOT byte-frozen by any golden test.
...
// (the inverse of marshalManifest), used by the restore read+verify pass.
