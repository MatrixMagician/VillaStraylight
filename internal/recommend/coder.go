...
// entry at its agent-profile context (m.AgentCtx, D-04 — the --ctx override is
// chat-only and is unreachable from here by construction) against the
// post-reservation envelope (D-05 ordering), and derives the residency mode as
// a PURE inequality output (D-06): "swap" when the best coder entry fits
...
type CoderFit struct {
	...
	// computed at (D-04) — never the --ctx chat override.
	AgentCtx int `json:"agent_ctx"`

	// The fit terms and verdict, mirroring the chat-pick inequality (D-06).
	...
	// standalone, "shared" when none does (D-06).
...
// considered, and ctx is ALWAYS m.AgentCtx (D-04). The total uses the
// saturating form (WR-07) so an overflowing KV term can never wrap to "fits".
func pickCoder(c catalog.Catalog, envelope uint64) CoderFit {
	...
	for i := range c.Models {
		m := c.Models[i]
		if m.Role != "coder" {
			continue // coder stage considers only role:"coder" entries (D-03)
		}
		if m.Bootstrap {
		...
		if m.MinEnvelopeBytes > 0 && envelope < m.MinEnvelopeBytes {
			continue // secondary floor guard, applied identically to pickBest (IN-01)
		}
		total := addSaturating(addSaturating(m.WeightBytes, kvCacheBytes(m, m.AgentCtx)), headroom)
		if total > envelope {
			continue // OOM guard: swap requires a PROVEN standalone fit (D-06)
		}
