...
// D-02 coexistence); "rocm-6.4.4" and "rocm-6.4.4-rocwmma" select the two additive
// digest-pinned ROCm 6.4.4 backends (D-01); "vulkan" selects the Vulkan RADV backend,
...
// same image-parameterized backendROCm delta (D-06); only the pinned digest (and the
...
// the ROCm-name set is enumerated (D-08): callers use it instead of comparing
