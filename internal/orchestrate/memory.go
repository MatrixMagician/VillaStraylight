...
// Like openwebui.go, neither villa-qdrant nor villa-embed
...
// qdrantImage is the digest-pinned Qdrant vector-store image (the OFFICIAL
// qdrant/qdrant org, the v1.18.2-UNPRIVILEGED variant that runs as USER_ID=1000 so the
// rootless-Podman UID / SELinux:Z storage mount stays writable). This is the
...
// orchestrate managed-service seam, mirroring OpenWebUIImage.
...
// from. It is a DELIBERATELY INDEPENDENT const: the literal is byte-identical to
...
const (
	...
	// config-resolved (config.QdrantAddr via memory.RenderView) and threaded into
	// buildQdrantView, so config is the single source of truth for the
	...
	// label at Qdrant's data dir. The unprivileged image (USER_ID=1000) writes
	...
	// here — both are config-resolved (config.EmbedAddr / config.EmbedPort via
	// memory.RenderView) and threaded into buildEmbedView/buildEmbedExec, so
	...
	// never a host bind.
	//
	// embedContextLen is a genuine pinned const (8192): there is no
	...
	// The pinned, LOAD-BEARING embedding dimension (768) is single-sourced in
...
// services — gating the memory-service starts on the rendered units, not solely
...
// buildQdrantVolumeView assembles the durable Qdrant named-volume view.
...
// a single space (NO shell interpolation). `--embeddings` (trailing s)
...
// embedPort is the config-resolved villa-embed /v1 port (config.EmbedPort via RenderView)
// so the served --port matches the port the proof probes. embedContextLen
// stays a genuine pinned const (8192, — no config field for it).
