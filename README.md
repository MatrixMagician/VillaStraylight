...
> Status: **v1.13 shipped** (tag `v1.13`). Built milestone by milestone, each addition keeping the zero-telemetry, loopback-only posture:
...
> - **v1.9**: **speculation, vision, and a device-access gate, each licensed by a measurement.** `speculation = ngram` renders llama-server's `ngram-mod` behind the inference seam, is resolved by `villa recommend` from a catalog qualification an on-hardware probe earned, and is swapped by `villa speculation set` on the same capture → prove → rollback transaction as `backend set`; `villa bench` reports draft acceptance beside pp and tg. A draft-model sidecar was measured too and loses on every seed entry (all MoE), so it did not ship then (ADR-0006); the first dense entry reopened it (ADR-0009). A catalog entry may declare a **vision projector** sidecar, pulled and verified with the model and reserved as its own fit term, so an image attached in Open WebUI is answered. Preflight gained **PRE-08**: can the invoking user open `/dev/kfd` and a render node, refused before anything is written with the exact group fix.
> - **v1.10**: **the draft sidecar, on the first dense catalog entry.** `qwen3.8-27b` joined the catalog with its geometry read off the GGUF header, and the draft ADR-0006 had rejected was measured again on it: the model's own multi-token-prediction head, shipped as a separate file, took cold generation from 11 to 20 tokens per second, and combined with `ngram-mod` it won every cell. `speculation = draft` renders that sidecar behind the inference seam; a `Draft` embeds the sidecar shape and adds a catalog-qualified `spec_type`, so MTP is a spec type rather than a mode. The residency proof reads the journal as model-load blocks and proves the draft as a second model, with a CPU-resident draft a FAIL (ADR-0009). Two bugs fell out of the proof and were fixed: a speculation refusal printed an OOM warning, and `recommend --save` rewrote the config from defaults.
> - **v1.11**: **the workspace agent.** `villa work <folder> "<instruction>"` runs one instruction against one registered folder inside a per-task libkrun microVM, on an internal network that reaches the served model and nothing else, with the pinned Crush harness driven by a bridge villa ships inside the VM. Reads are free, every write asks unless `--auto`, and deletion asks in every mode as a command pattern, not a proof. After the run the same model audits every document the task wrote against the files it read; a claim it cannot support flags the task (exit `2`), and a clean audit means the auditor found none. The runner lives in the dashboard service; `villa task` and the dashboard's panels answer approvals. `villa tools-mode enter|exit` serves the chat unit for tool calling on the same capture → prove → rollback transaction as `backend set`, with a context floor measured on hardware, and `villa sandbox build` builds the task image on this host and records its digest as the effective pin. Every number the spec left open was measured on the live stack before the tag: the ctx floor, the VM memory default and the audit's strictness.
> - **v1.12**: **the first signed pin manifest, and proofs that assert the answer.** The compiled-in pins were re-vetted on hardware (six moved and were proven; the rebuilt `rocm-7.2.4` tag was refused because it streams garbage under n-gram speculation), and the first manifest is published: serial 2, valid for twelve months, signed offline, attached to the release, so `villa update --check` returns a real verdict instead of "could not check". The release workflow carries the manifest forward to each new tag without a key in CI. The cutover generation probe that every transactional verb gates on now asserts the answer, not the arrival: a reply without `ok`, or one repeated byte, rolls the cutover back. `villa up` restarts a running service whose unit changed instead of writing a unit nothing applied, and `villa doctor` reports the last recorded update check beside `status`.
> - **v1.13**: **the dashboard reskinned, the catalog refreshed, and a gate that reads its own probe.** The control dashboard is rebuilt in the Neuromancer-noir language with two read-only endpoints: `GET /api/pins` shows every component as tag@digest-head, never the tag alone, and `GET /api/journal` is one scoped `journalctl --user` tail over villa's units. Catalog 2026.09.4 refreshes the bootstrap and validation entries to measured Qwen3.5 GGUFs. The ROCm preflight gate now reads the linux-firmware date and HSA viability that `villa detect` already probed, so on real hardware `ROCM-PRE-firmware` and `ROCM-PRE-hsa` PASS or FAIL instead of warning forever; `firmware_date` joins `HostProfile` as schema 4. `make dev-deploy` runs the dev-host build, restart and doctor sequence.

## Requirements

- **Go 1.26+** to build the `villa` binary (see `go.mod`).
...
| `ttm.pages_limit=32505856` | **Pinned-memory limit:** allows the TTM (Translation Table Manager) to pin up to ~124 GB of pages in high-speed system RAM, so the GPU reads them directly without swapping. |
