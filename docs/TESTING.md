...
- **`TestDocsNoRetiredClaims`**: a line-scoped denylist of claims that were
  **true once**. Each entry carries the reason it was retired, which is what the
  failure prints. `retiredClaims` in the test is the list; it is deliberately
...
the doc against the code. The structural defence against that class is to stop
writing enumerations into prose, which is why the docs now point
...
- `offload_test.go` runs `scrapeOffloadLog` against captured `*.stderr` fixtures:
...
- `running_offload_test.go` covers the already-running-server verdict: residency
...
config actually loads and runs on a real AMD Strix Halo (gfx1151), the
v1.1 ROCm backend and the honest A/B benchmark included, is a **separate, manual
