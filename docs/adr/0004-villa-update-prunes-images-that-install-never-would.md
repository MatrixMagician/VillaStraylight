...
"move this component from digest A to digest B", so A is the state being changed,
and therefore the state a rollback must restore. An
...
two, and for them the three members above alone are no rollback target: a
