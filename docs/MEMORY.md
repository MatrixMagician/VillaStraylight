...
memory is driven by one of two opt-in mechanisms rather than a single env var, and the
"default-off" guarantee is enforced simply by `villa` **not enabling either of them**:

1. **Native Function Calling (Agentic Mode), the built-in path.** Enable
...
villa-managed Knowledge collection (the literal name is `Villa Recall — Past Conversations`) through
...
- `stale`: new/changed/deleted counts versus the **live** chat list. If Open WebUI
  ...
  the literal string `Unknown — could not evaluate`. Unknown is **not** "current", and it is never
  rendered as `0`.
